...
#include <XBOPTools_RoughShapeIntersector.ixx>
...
static Bnd_Box GetBoxEnlargedBySubShape(const Standard_Integer                        theIndex,
					const XBooleanOperations_PShapesDataStructure& PDS);


// =============================================================================================
// function: Constructor
// =============================================================================================
  XBOPTools_RoughShapeIntersector::XBOPTools_RoughShapeIntersector(const XBooleanOperations_PShapesDataStructure& PDS)
:
...
// function: Perform
// =============================================================================================
  void XBOPTools_RoughShapeIntersector::Perform() 
{
  ...
  // 
	...
	for(anIt2.Initialize(aSortedByTypeShapes2.Value(index2)); anIt2.More(); anIt2.Next()) {
	  ...
	  // 
	  XBOPTools_IntersectionStatus aStatus = myTableOfStatus->Value(anIt1.Value(), anIt2.Value());

	  if(aStatus != XBOPTools_UNKNOWN && aStatus != XBOPTools_BOUNDINGBOXOFSUBSHAPESINTERSECTED)
	    continue;

	  const Bnd_Box& B1 = myBoundingBoxes->Value(anIt1.Value());
	  const Bnd_Box& B2 = myBoundingBoxes->Value(anIt2.Value());
	  // 
	  XBOPTools_IntersectionStatus aNewValue = XBOPTools_BOUNDINGBOXINTERSECTED;

	  if(B1.IsOut(B2)) {
	    aNewValue = XBOPTools_NONINTERSECTED;
	    // 
	    PropagateForSuccessors1(anIt1.Value(), anIt2.Value(), aNewValue);
	    //
	  }
	  else {
	    aNewValue = XBOPTools_BOUNDINGBOXINTERSECTED;
	    const Bnd_Box& BB1 = myPDS->GetBoundingBox(anIt1.Value());
	    const Bnd_Box& BB2 = myPDS->GetBoundingBox(anIt2.Value());

	    if(BB1.IsOut(BB2)) {
	      aNewValue = XBOPTools_BOUNDINGBOXOFSUBSHAPESINTERSECTED;
	    }
	  }
...
// =============================================================================================
const Handle(XBOPTools_HArray2OfIntersectionStatus)& XBOPTools_RoughShapeIntersector::TableOfStatus() const
{
...
// function: Prepare
// =============================================================================================
void XBOPTools_RoughShapeIntersector::Prepare() 
{
  myIsDone = Standard_False;
  ...
  if(!myTableOfStatus.IsNull() &&
    ...
    if(myPDS != NULL) {
      ...
      myTableOfStatus = new XBOPTools_HArray2OfIntersectionStatus(firstlowerindex, firstupperindex, 
  ...
  myTableOfStatus->Init(XBOPTools_UNKNOWN);  
  // 
...
// function: IsDone
// =============================================================================================
  Standard_Boolean XBOPTools_RoughShapeIntersector::IsDone() const
{
  return myIsDone;
}
...
// =============================================================================================
void XBOPTools_RoughShapeIntersector::PropagateForSuccessors1(const Standard_Integer            AncestorsIndex1,
							     const Standard_Integer            AncestorsIndex2,
							     const XBOPTools_IntersectionStatus theStatus) 
{
  if(myTableOfStatus->Value(AncestorsIndex1, AncestorsIndex2) != XBOPTools_UNKNOWN)
    return;
...
// =============================================================================================
void XBOPTools_RoughShapeIntersector::PropagateForSuccessors2(const Standard_Integer            AncestorsIndex1,
							     const Standard_Integer            AncestorsIndex2,
							     const XBOPTools_IntersectionStatus theStatus) 
{
  for(Standard_Integer j = 1; j <= myPDS->NumberOfSuccessors(AncestorsIndex2); j++) {
    ...
    if(myTableOfStatus->Value(AncestorsIndex1, asuccessor2) == XBOPTools_UNKNOWN) {
