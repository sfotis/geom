...
static
  Standard_Boolean IsPaveBlock(const Standard_Integer nV1,
			       ...
			       const XBOPTools_ListOfPaveBlock& aLPBExisting);
static
...
static
  void FMapWith(const Standard_Integer nF,
		XBOPTools_InterferencePool* myIntrPool,
...
//wkar OCC334 f
static
  void UnUsedMap(XBOPTools_SequenceOfCurves& aSCvs,
		 const XBOPTools_PaveSet& aPSF,
...
static
  void ProcessAloneStickVertices(const Standard_Integer nF1,
				 const Standard_Integer nF2,
				 const XBOPTools_PaveSet& aPSF,
				 XBOPTools_SequenceOfCurves& aSCvs,
				 const XBOPTools_InterferencePool& anIntrPool,
				 XBOPTools_PaveFiller& aPF,
...
static
  Standard_Boolean IsPairFound(const Standard_Integer nF1,
			       const Standard_Integer nF2,
			       XBOPTools_InterferencePool* myIntrPool,
			       XBOPTColStd_IndexedDataMapOfIntegerIndexedMapOfInteger& aMapWhat,
			       XBOPTColStd_IndexedDataMapOfIntegerIndexedMapOfInteger& aMapWith);
...
static Standard_Boolean CheckNewVertexAndUpdateData(const TopoDS_Vertex&              theVertex,
						    ...
						    const XBOPTools_PInterferencePool& theIntrPool,
						    const XBooleanOperations_PShapesDataStructure& theDS,
						    const Handle(XIntTools_Context)&   theContext,
						    const XBOPTools_PaveSet&           theEdgePaveSet,
						    const Standard_Boolean            bAddNewVertex,
						    const Standard_Boolean            bAddOldVertex,
						    XBOPTools_Curve&                   theBC,
						    XBOPTools_Pave&                    thePaveToPut,
...
static void AddInterfForAdjacentFace(const Standard_Integer            theEdgeIndex,
				     const Standard_Integer            theIndexF1,
				     const Standard_Integer            theIndexF2,
				     XBOPTools_PInterferencePool        theIntrPool,
				     const XBooleanOperations_PShapesDataStructure& theDS);

static Standard_Boolean RejectPaveBlock(const XIntTools_Curve& theC,
...
static Standard_Integer RejectBuildingEdge(const XIntTools_Curve& theC,
...
static
  void CorrectTolR3D(XBOPTools_PaveFiller& aPF,
		     const XBOPTools_SSInterference& aFF,
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PerformFF() 
{
  myIsDone=Standard_False;
  Standard_Boolean bIsFound, bToSplit;
  Standard_Integer n1, n2, anIndexIn=0, nF1, nF2, aNbFFs, aBlockLength;
  XBOPTColStd_IndexedDataMapOfIntegerIndexedMapOfInteger aMapWhat, aMapWith;
  //
  XBOPTools_CArray1OfSSInterference& aFFs=myIntrPool->SSInterferences();
  //
  ...
  for (; myDSIt.More(); myDSIt.Next()) {
	...
	//
	if (!bIsFound) {
	  myIntrPool->AddInterference (nF1, nF2, XBooleanOperations_SurfaceSurface, anIndexIn); 
	}
	else {
	  XIntTools_SequenceOfPntOn2Faces aPnts;
	  XIntTools_SequenceOfCurves aCvs;

	  XBOPTools_SSInterference anInterf (nF1, nF2, 1.e-07, 1.e-07, aCvs, aPnts);
	...
	for(Standard_Integer interIt = 1; interIt <= aEFs.Length(); interIt++) {
	  const XBOPTools_ESInterference& aEF = aEFs(interIt);          

	  if((aEF.Index1() == nIndexE) && (nFOpposite == aEF.Index2())) {
	    XIntTools_CommonPrt aCP = aEF.CommonPrt();
	...
	if (!bIsFound) {
      ...
      //
      XBOPTools_SSInterference anInterf (nF1, nF2, aTolR3D, aTolR2D, aCvs, aPnts);
      anIndexIn=aFFs.Append(anInterf);
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::MakeBlocks()
{
  ...
  for (i=1; i<=aNbFFs; i++) {
    ...
    XBOPTools_SequenceOfCurves& aSCvs = aFFi.Curves();
    ...
    for (j=1; j<=aNbCurves; j++) {
      XBOPTools_Curve& aBC=aSCvs(j);
      const XIntTools_Curve& aC= aBC.Curve();
      
      XBOPTools_PaveSet& aPaveSet=aBC.Set();
      XBOPTools_PaveBlockIterator aPBIter(0, aPaveSet);
      for (; aPBIter.More(); aPBIter.Next()) {
	XBOPTools_PaveBlock& aPBNew=aPBIter.Value();
	aPBNew.SetCurve(aC);
	aPBNew.SetFace1(nF1);
	aPBNew.SetFace2(nF2);
	//
	...
	if (aMap.Contains(nV1) && aMap.Contains(nV2)) {
	  //
	  const XBOPTools_ListOfPaveBlock& aLPBExisting=aFFi.PaveBlocks();
	  bIsPaveBlock=IsPaveBlock(nV1, nV2, aLPBExisting);
	  //
	  iCheckIntermediatePoint=1;
	  if (bIsPaveBlock) {
	    XBOPTools_ListIteratorOfListOfPaveBlock anItLPB(aLPBExisting);
	    
	    for (; anItLPB.More(); anItLPB.Next()) {
	      const XBOPTools_PaveBlock& aPBR=anItLPB.Value();
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::MakeAloneVertices()
{
  ...
  for (i=1; i<=aNbFFs; i++) {
    ...
    PrepareSetForFace (nF1, nF2, aPSF);
    const XBOPTools_ListOfPave& aLPaves=aPSF.Set();
    ...
    for (j=1; j<=aNbCurves; j++) {
      XBOPTools_Curve& aBC=aSCvs(j);
      const XBOPTools_PaveSet& aCPSF=aBC.Set();
      const XBOPTools_ListOfPave& aLPs=aCPSF.Set();
      anIt.Initialize(aLPs);
      for (; anIt.More(); anIt.Next()) {
	const XBOPTools_Pave& aPv=anIt.Value();
	nV=aPv.Index();
	aMap.Add(nV);
      }
    }
    ...
    for (j=1; j<=aNbAlone; ++j) {
      const XIntTools_PntOn2Faces& aP2F=aSeqAlonePnts(j);
      const XIntTools_PntOnFace& aPF1=aP2F.P1();
	...
	iVV= XIntTools_Tools::ComputeVV (aVAlone, aV);
	if (!iVV) {
...
// purpose: 
//=======================================================================
  Standard_Boolean XBOPTools_PaveFiller::CheckCoincidence(const XBOPTools_PaveBlock& aPB,
							 const XBOPTools_SSInterference& aFFi)
							 
{
  ...
  aTolC=aFFi.TolR3D();
  // V11
  const XBOPTools_Pave& aPave11=aPB.Pave1();
  nV11=aPave11.Index();
  const TopoDS_Vertex& aV11=TopoDS::Vertex(myDS->GetShape(nV11));
  aT11=aPave11.Param();
  // V12
  const XBOPTools_Pave& aPave12=aPB.Pave2();
  nV12=aPave12.Index();
  const TopoDS_Vertex& aV12=TopoDS::Vertex(myDS->GetShape(nV12));
  aT12=aPave12.Param();
  //
  const XBOPTools_ListOfPaveBlock& aLPB=aFFi.PaveBlocks();
  XBOPTools_ListIteratorOfListOfPaveBlock anIt(aLPB);
  for (; anIt.More(); anIt.Next()) {
    
    iCount=0;
    
    const XBOPTools_PaveBlock& aPBR=anIt.Value();
    // V21
    const XBOPTools_Pave& aPave21=aPBR.Pave1();
    nV21=aPave21.Index();
    const TopoDS_Vertex& aV21=TopoDS::Vertex(myDS->GetShape(nV21));
    // V22
    const XBOPTools_Pave& aPave22=aPBR.Pave2();
    nV22=aPave22.Index();
    ...
    // VV
    iV=0;
    iVV=XIntTools_Tools::ComputeVV (aV11, aV21);
    if (!iVV) {
      iCount++;
      iV++;
      if (iCount>iCountExt) {
    ...
    iVV=XIntTools_Tools::ComputeVV (aV11, aV22);
    if (!iVV) {
      iCount++;
      iV++;
      if (iCount>iCountExt) {
    ...
    // VV
    iV=0;
    iVV=XIntTools_Tools::ComputeVV (aV12, aV21);
    if (!iVV) {
      iCount++;
      iV++;
      if (iCount>iCountExt) {
    ...
    iVV=XIntTools_Tools::ComputeVV (aV12, aV22);
    if (!iVV) {
      iCount++;
      iV++;
      if (iCount>iCountExt) {
...
// purpose: 
//=======================================================================
  Standard_Integer XBOPTools_PaveFiller::CheckIntermediatePoint(const XBOPTools_PaveBlock& aPB,
							 ...
							 
{
  ...
  aT12=aPave12.Param();
  //
  aTM=XIntTools_Tools::IntermediatePoint (aT11, aT12);
  //
  const XIntTools_Curve& aIC=aPB.Curve();
  aIC.D0(aTM, aPM);
  //
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutBoundPaveOnCurve(XBOPTools_Curve& aBC,
						XBOPTools_SSInterference& aFFi)
{ 
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutBoundPaveOnCurve(const gp_Pnt& aP,
...
{ 
  ...
  XBOPTools_Pave aPave1, aPave2, aPave;
  XBOPTools_PaveSet& aCPS=aBC.Set();
  XBOPTools_PaveSet& aFFiPS=aFFi.NewPaveSet();
  const XIntTools_Curve& aIC=aBC.Curve();
  ...
  if (!bFound1 && !bFound2) {
    ...
    XBOPTools_Tools::MakeNewVertex(aP, aTolV, aNewVertex);
    //
    XBooleanOperations_AncestorsSeqAndSuccessorsSeq anASSeq;
  ...
  if (bFound1 && !bFound2) {
    ...
    //
    const TopoDS_Vertex& aV=TopoDS::Vertex(myDS->Shape(nV));
    XBOPTools_Tools::UpdateVertex (aIC, aT, aV);
  }
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutBoundPaveOnCurveSpec(XBOPTools_Curve& aBC,
						    XBOPTools_SSInterference& aFFi)
{ 
  Standard_Boolean bHasBounds, bVF;
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutBoundPaveOnCurveSpec(const gp_Pnt& aP,
						    const Standard_Real aT,
						    XBOPTools_Curve& aBC,
						    XBOPTools_SSInterference& aFFi)
{ 
  ...
  if (!bFound1 && !bFound2) {
	...
	myContext->ComputeVE(aNewVertex, TopoDS::Edge(anE), aPar);
      ...
      XBOPTools_Tools::MakeNewVertex(aP1, aTolV, aNewVertex1);
      anErrStat = 
	myContext->ComputeVE(aNewVertex1, TopoDS::Edge(anE), aT1);
      ...
      XBOPTools_VEInterference anInterf (aWhat, aWith, aPar);
      anIndexIn=aVEs.Append(anInterf);
      //
      // Add Pave to the Edge's myPavePool
      XBOPTools_Pave aPave3(aWhat, aPar, XBooleanOperations_VertexEdge);
...
// purpose: 
//=======================================================================
  Standard_Boolean XBOPTools_PaveFiller::FindPave(const gp_Pnt& aP,
						 ...
						 XBOPTools_Pave& aPave)
{
  ...
  const XBOPTools_ListOfPave& aLP=aPS.Set();
  XBOPTools_ListIteratorOfListOfPave anIt(aLP);
  for (; anIt.More(); anIt.Next()) {
    const XBOPTools_Pave& aPC=anIt.Value();
    nV=aPC.Index();
    const TopoDS_Vertex& aV=TopoDS::Vertex(myDS->GetShape(nV));
    bIsVertex=XIntTools_Tools::IsVertex (aP, aTolPV, aV);
    if (bIsVertex) {
      aPave=aPC;
      return bIsVertex;
    }
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::MakeSectionEdges()
{
  ...
  for (i=1; i<=aNbFFs; i++) {
    ...
    XBOPTools_ListOfPaveBlock aFFPBL;
          ...
          if(eI != 0) {
            ...
            for(; aSEIt.More(); aSEIt.Next()) {
              ...
              if(eIndex == eI) {
                ...
                XBOPTools_ListOfPaveBlock& aListPB = (XBOPTools_ListOfPaveBlock&) aSectEdges;
	...
	XBOPTools_Tools::MakeSectEdge (aIC, aV1, t1, aV2, t2, aESect);
	//
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutPaveOnCurve(const XBOPTools_PaveSet& aPaveSet,
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutPaveOnCurve (const XBOPTools_Pave& aPave,
					    const Standard_Real aTolR3D,
					    XBOPTools_Curve& aBC)
{
  Standard_Integer nV;
  ...
  //
  if (bIsVertexOnLine) {
    XBOPTools_Pave aPaveNew(nV, aT, XBooleanOperations_SurfaceSurface);
    XBOPTools_PaveSet& aPaveSet=aBC.Set();
    aPaveSet.Append(aPaveNew);
    //<-B
    XBOPTools_Tools::UpdateVertex (aC, aT, aV);
...
// purpose: 
//=======================================================================
  Standard_Boolean XBOPTools_PaveFiller::ExtendedTolerance(const Standard_Integer nV,
							  Standard_Real& aTolVExt)
{
  ...
  //
  const XBOPTools_InterferencePool& anInterferencePool=*myIntrPool;
  const XBOPTools_CArray1OfEEInterference&  aEEInterfs=anInterferencePool.EEInterfs();
  const XBOPTools_CArray1OfESInterference&  aESInterfs=anInterferencePool.ESInterfs();
  //
  for (k=0; k<2; ++k) {
    aNbLines=(!k) ? aEEInterfs.Extent() : aESInterfs.Extent();
    for (i=1; i<=aNbLines; ++i) {
      XBOPTools_ShapeShapeInterference *pI=(!k) ? 
	(XBOPTools_ShapeShapeInterference *)&aEEInterfs(i):
	  (XBOPTools_ShapeShapeInterference *)&aESInterfs(i);
      //
      aNewShape=pI->NewShape();
      if (aNewShape==nV) {
	const XIntTools_CommonPrt& aCPart=(!k) ? 
	  aEEInterfs(i).CommonPrt() : 
	    aESInterfs(i).CommonPrt();
	//
	const TopoDS_Edge& aE1=aCPart.Edge1();
	aCPart.Range1(aT11, aT12);
	XBOPTools_Tools::PointOnEdge(aE1, aT11, aP11);
	XBOPTools_Tools::PointOnEdge(aE1, aT12, aP12);
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PutPavesOnCurves ()
{
  ...
  for (i=1; i<=aNbFFs; i++) {
    ...
    //f
    {
      ...
      XBOPTools_ListIteratorOfListOfPave aItLP;
      TColStd_MapOfInteger aMVStick;
      //
      const XBOPTools_ListOfPave& aLPX=aPSF.Set();
      aItLP.Initialize(aLPX);
      for (; aItLP.More(); aItLP.Next()) {
	const XBOPTools_Pave& aPX=aItLP.Value();
	nVX=aPX.Index();
	aMVStick.Add(nVX);
      }
      //
    ...
    for (j=1; j<=aNbCurves; j++) {
      XBOPTools_Curve& aBC=aSCvs(j);
      XBOPTools_ListOfPave anOldList;
	...
	continue;
      ...
      XBOPTools_CArray1OfESInterference& aESs = myIntrPool->ESInterferences();
	...
	XBOPTools_ESInterference& aES = aESs(k);
	...
	for(; anExp.More(); anExp.Next()) {
	  ...
	  if(nE) {
	    const XBOPTools_ListOfInterference& aList =
	      myIntrPool->InterferenceTable().Value(nE).GetOnType(XBooleanOperations_EdgeSurface);
	    XBOPTools_ListIteratorOfListOfInterference anIt2(aList);
      ...
      XBOPTools_ListOfPave aListOfNewPave;
      XBOPTools_ListIteratorOfListOfPave anIt1, anIt2;
	...
	if(!bfound) {
      ...
      for (m=1; m<=aNbFFs; m++) {
	XBOPTools_SSInterference& aFFm = aFFs(m);
	// 
	// Curves' tolerance
	Standard_Real aTolR3D2 = aFFm.TolR3D();

	XBOPTools_SequenceOfCurves& aSCvs2 = aFFm.Curves();
	Standard_Integer aNbCurves2 = aSCvs2.Length();

	for(n = 1; n <= aNbCurves2; n++) {
	  if((n == j) && (m == i))
	    continue;
	  XBOPTools_Curve& aBC2 = aSCvs2(n);
...
// purpose: 
//=======================================================================
  void XBOPTools_PaveFiller::PrepareSetForFace(const Standard_Integer nF1,
					      const Standard_Integer nF2,
					      XBOPTools_PaveSet& aPaveSet)
{ 
			     ...
			     const XBOPTools_ListOfPaveBlock& aLPBExisting)
{
  ...
  XBOPTools_ListIteratorOfListOfPaveBlock anIt(aLPBExisting);
  for (; anIt.More(); anIt.Next()) {
    const XBOPTools_PaveBlock& aPBR=anIt.Value();
    // 
		    ...
		    
{
  ...
  XBooleanOperations_ShapesDataStructure* myDS=myIntrPool->DS();
		...
		TColStd_IndexedMapOfInteger& aMapWith)
{
  ...
  const XBOPTools_CArray1OfInterferenceLine& anArrIL= myIntrPool->InterferenceTable();

  aNb=aMapWhat.Extent();
  for (i=1; i<=aNb; i++) {
    aWhat=aMapWhat(i);
    
    const XBOPTools_InterferenceLine& aWithLine=anArrIL(aWhat);
  
    const XBOPTools_ListOfInterference& aLI=aWithLine.List();
    anIt.Initialize(aLI);
    for (; anIt.More(); anIt.Next()) {
      const XBOPTools_Interference& anIntf=anIt.Value();
			       ...
			       TColStd_SequenceOfReal& aSeqTolVx)
{
  ...
  //
  XBooleanOperations_PShapesDataStructure pDS=anIntrPool.DS();
  //
  ...
  if((aType1==GeomAbs_Torus && aType2==GeomAbs_Plane) ||
    ...
    aNbVtx=aMapUnUsed.Extent();
    if (aNbVtx) {
      const Handle(XIntTools_Context)& aCtx=aPF.Context();
      //
      aNbSCvs=aSCvs.Length();
      if (aNbSCvs==1) {
	XBOPTools_Curve& aBC=aSCvs(1);
	const XIntTools_Curve& aIC=aBC.Curve();
	Handle (Geom_Curve) aC3D= aIC.Curve();
	//
	bIsClosed=XIntTools_Tools::IsClosed(aC3D);
	if (bIsClosed) {
	  return;
	}
			       ...
			       TColStd_SequenceOfReal& aSeqTolVx)
{
  GeomAbs_SurfaceType aType1, aType2;
  //
  XBooleanOperations_PShapesDataStructure pDS=aPF.DS();
  //
  const TopoDS_Face& aF1= TopoDS::Face(pDS->Shape(nF1));
  const TopoDS_Face& aF2= TopoDS::Face(pDS->Shape(nF2));
  Handle(Geom_Surface) aS1=BRep_Tool::Surface(aF1);
  ...
  if(aType1==GeomAbs_Torus  || aType2==GeomAbs_Torus) {
	...
	for (m=0; m<2; ++m) {
	  ...
	  for (n=0; n<2; ++n) {
	    ...
	    aP2D.Coord(aU, aV);
	    XBOPTools_Tools3D::GetNormalToSurface(aS, aU, aV, aDN[n]);
	  }
	  // 
	  aScPr=aDN[0]*aDN[1];
	  if (aScPr<0.) {
	       ...
	       TColStd_IndexedMapOfInteger& aMapUnUsed)
{
  ...
  XBOPTools_ListIteratorOfListOfPave anLPIt;

  aNbCurves=aSCvs.Length();
  for (j=1; j<=aNbCurves; ++j) {
    XBOPTools_Curve& aBC=aSCvs(j);
    //const XIntTools_Curve& aC= aBC.Curve();// Wng in Gcc 3.0
	
    const XBOPTools_PaveSet& aPaveSet=aBC.Set();
    const XBOPTools_ListOfPave& aLPAlreadyUsed=aPaveSet.Set();
    anLPIt.Initialize(aLPAlreadyUsed);
    for (; anLPIt.More(); anLPIt.Next()) {
      const XBOPTools_Pave& aPave=anLPIt.Value();
      nV1=aPave.Index();
      aMapUsed.Add(nV1);
    }
  }
  ...
  const XBOPTools_ListOfPave& aLPMustUsed=aPSF.Set();
				      ...
				      Standard_Real& aTolV)
{
  ...
  //
  // EF Interferences
  const XBOPTools_CArray1OfESInterference& aEFs=anIntrPool.ESInterfs();
  ...
  for (i=1; i<=aNbEFs; ++i) {
	...
	//
	const XIntTools_CommonPrt& aCommonPrt=aEF.CommonPrt();
  ...
  // EE Interferences
  /*
  const XBOPTools_CArray1OfEEInterference& aEEs=anIntrPool.EEInterfs();
  aNbEEs=aEEs.Extent();
  for (i=1; i<=aNbEEs; ++i) {
    const XBOPTools_EEInterference& aEE=aEEs(i);
...
//=======================================================================
  void XBOPTools_PaveFiller::RestrictCurveIn2d(const Standard_Integer nE,
					      const Standard_Integer nF1,
					      const Standard_Integer nF2,
					      const Standard_Real    theTolerance,
					      XBOPTools_Curve&        theBC) 
{
  ...
  XBOPTools_ListIteratorOfListOfPave anIt1(aPS.Set());
  XBOPTools_ListIteratorOfListOfPave anIt2;
...
//             gp_Pnt2d aP2d = aInt.Point(jj);
  ...
  if(bSearchInter) {
    ...
    XBOPTools_ListOfPave aPavesOnCurve, aPavesOnEdge;
	...
	if((t1 >= f1) && (t1 <= l1) &&
	  ...
	  if(myContext->IsPointInOnFace(TopoDS::Face(aFace), aP2dOnFace) &&
	    ...
	    XBOPTools_Pave aPave1;
	    aPave1.SetParam(t1);
	    aPave1.SetIndex(-1);
	    aPavesOnCurve.Append(aPave1);
	    XBOPTools_Pave aPave2;
	    aPave2.SetParam(t2);
	    aPave2.SetIndex(-1);
    ...
    XBOPTools_ListIteratorOfListOfPave aPaveIt1(aPavesOnCurve);
    XBOPTools_ListIteratorOfListOfPave aPaveIt2(aPavesOnEdge);

    // test common blocks. begin
    Standard_Boolean bFaceCBFound = Standard_False;
    Standard_Boolean bEdgeCBFound = Standard_False;
    const XBOPTools_ListOfCommonBlock& aLCBTest = myCommonBlockPool(myDS->RefEdge(nE));
    XBOPTools_ListIteratorOfListOfCommonBlock aCBListIt(aLCBTest);
    ...
    if(!bHasCBOnFace || !bHasCBOnEdge) {
      XBOPTools_PaveSet aTestPaveSet;
      aTestPaveSet.ChangeSet() = aPS.Set();

      for(; aPaveIt2.More(); aPaveIt2.Next()) {
	aTestPaveSet.Append(aPaveIt2.Value());
      }
      XBOPTools_PaveBlockIterator aPBIter(0, aTestPaveSet);

      for (; aPBIter.More(); aPBIter.Next()) {
	const XBOPTools_PaveBlock& aPB = aPBIter.Value();
	...
	XBOPTools_Pave& aPaveOnCurve = aPaveIt1.Value();
	XBOPTools_Pave& aPaveOnEdge  = aPaveIt2.Value();
	...
	if(bAddNewVertex || bAddOldVertex) {
	  TopoDS_Vertex aNewVertex;
	  XBOPTools_Tools::MakeNewVertex(anEdge, aPaveOnEdge.Param(), 
					TopoDS::Face(aFace),  aNewVertex);
	  
	  XBOPTools_Pave aPaveToPut;
	...
	if(bAddNewVertex || (bAddOldVertex && oldvertexindex)) {
	  ...
	  Standard_Integer aNewShapeIndex = 0;
          ...
          XBOPTools_Tools::MakeNewVertex(anEdge,aPaveOnEdge.Param(),TopoDS::Face(aFace),aNewVertex);
	  ...
	  if(!bAddOldVertex) {
	    ...
	    //
          ...
          XBOPTools_ListIteratorOfListOfPave anItAll;
          ...
          for(anItAll.Initialize(aPS.Set()); anItAll.More(); anItAll.Next()) {
            if(anItAll.Value().Index() == aNewShapeIndex) {
              XBOPTools_Pave& aPV = anItAll.Value();
	  ...
	  myDS->SetState (aNewShapeIndex, XBooleanOperations_ON);

	  XIntTools_CommonPrt aCPart;
	  aCPart.SetEdge1(anEdge);
	  aCPart.SetType(TopAbs_VERTEX);
	  aCPart.SetRange1(XIntTools_Range(aPaveOnEdge.Param(), aPaveOnEdge.Param()));
	  aCPart.SetVertexParameter1(aPaveOnEdge.Param());

	  Standard_Integer nFOpposite = (bIsOnFirst) ? nF2 : nF1;
	  XBOPTools_ESInterference anInterf (nE, nFOpposite, aCPart);
	  Standard_Integer anIndexIn = aESs.Append(anInterf);
	  myIntrPool->AddInterference (nE, nFOpposite, XBooleanOperations_EdgeSurface, anIndexIn);

	  XBOPTools_ESInterference& aESInterf = aESs(anIndexIn);
	  aESInterf.SetNewShape(aNewShapeIndex);

	  // put pave on edge. begin 
	  aPaveOnEdge.SetInterference(anIndexIn);
	  aPaveOnEdge.SetType (XBooleanOperations_EdgeSurface);
	  aPaveOnEdge.SetIndex(aNewShapeIndex);

	  XBOPTools_PaveSet& aPaveSet1 = myPavePoolNew(myDS->RefEdge(nE));
	  aPaveSet1.Append(aPaveOnEdge);
	  // put pave on edge. end

	  // put pave on curve. begin 
	  aPaveOnCurve.SetIndex(aNewShapeIndex);
	  aPaveOnCurve.SetType(XBooleanOperations_SurfaceSurface);
	  XBOPTools_PaveSet& aPaveSet = theBC.Set();
	  aPaveSet.Append(aPaveOnCurve);

	  XBOPTools_Tools::UpdateVertex (theBC.Curve(), aPaveOnCurve.Param(), aNewVertex);

	  // put pave on curve. end

	  XBOPTools_ListOfCommonBlock& aLCB1 = myCommonBlockPool(myDS->RefEdge(nE));

	  XBOPTools_ListIteratorOfListOfCommonBlock  anIt(aLCB1);

	  for(; anIt.More(); anIt.Next()) {
	    XBOPTools_CommonBlock& aCB = anIt.Value();
	    ...
	    XIntTools_Range aRange = aCB.PaveBlock2().Range();
		...
		XBOPTools_Pave aPaveOpp;
		aPaveOpp.SetParam(aOppParameter);
		aPaveOpp.SetIndex(aNewShapeIndex);
		XBOPTools_PaveSet& aPaveSetOpp = myPavePoolNew(myDS->RefEdge(anOppIndex));
...
//purpose  : 
//=======================================================================
  void XBOPTools_PaveFiller::RecomputeCommonBlocks(const Standard_Integer nE) 
{
  TopoDS_Shape atmpShape = myDS->Shape(nE);
  TopoDS_Edge  anEdge   = TopoDS::Edge(atmpShape);
  XBOPTools_ListOfCommonBlock& aLCB1 = myCommonBlockPool(myDS->RefEdge(nE));

  XBOPTools_ListOfCommonBlock anOldLCB;
  anOldLCB = aLCB1;
  aLCB1.Clear();
  XBOPTools_ListOfCommonBlock aNewLCB1;
  Standard_Boolean bReverse = Standard_False;
  XBOPTools_ListIteratorOfListOfCommonBlock  anIt(anOldLCB);

  for(; anIt.More(); anIt.Next()) {
    XBOPTools_CommonBlock& anOldCB = anIt.Value();

    Standard_Integer anIndex = anOldCB.PaveBlock1().OriginalEdge();
    Standard_Integer anIndexOpp = anOldCB.PaveBlock2().OriginalEdge();
    XIntTools_Range aCBRange = anOldCB.PaveBlock1().Range();
    ...
    XBOPTools_ListOfPaveBlock& aSplitEdges1 = mySplitShapesPool(myDS->RefEdge(nE));
    XBOPTools_ListIteratorOfListOfPaveBlock aLPBIt1(aSplitEdges1);
    Standard_Boolean found = Standard_False;
    XBOPTools_ListOfCommonBlock aNewListOfCommonBlock;

    for(; aLPBIt1.More(); aLPBIt1.Next()) {
      XBOPTools_PaveBlock& aPBCurrent1 = aLPBIt1.Value();
      XIntTools_Range aCurRange1 = aPBCurrent1.Range();
	...
	if(anOldCB.Face()) {
	  ...
	  if(anIndexOpp <= 0)
	    continue;
	  XBOPTools_ListIteratorOfListOfPaveBlock aLPBIt2(mySplitShapesPool(myDS->RefEdge(anIndexOpp)));

	  for(; aLPBIt2.More(); aLPBIt2.Next()) {
	    XBOPTools_PaveBlock& aPBCurrent2 = aLPBIt2.Value();
	    XIntTools_Range aCurRange2 = aPBCurrent2.Range();
    ...
    if(!found) {
      ...
      XBOPTools_ListOfCommonBlock tmplst;
      tmplst = aNewListOfCommonBlock;
      aNewLCB1.Append(tmplst);

      if((anOldCB.Face() == 0) && (anIndexOpp > 0)) {
	tmplst = aNewListOfCommonBlock;
	XBOPTools_ListOfCommonBlock& aLCB2 = myCommonBlockPool(myDS->RefEdge(anIndexOpp));

	XBOPTools_ListIteratorOfListOfCommonBlock anItLCB2(aLCB2);

	for(; anItLCB2.More(); anItLCB2.Next()) {
	  XBOPTools_CommonBlock& anOldCB2 = anItLCB2.Value();
...
Standard_Boolean CheckNewVertexAndUpdateData(const TopoDS_Vertex&              theVertex,
  ...
  for(anIt1.Initialize(theEdgePaveSet.Set()); anIt1.More(); anIt1.Next()) {
    ...
    if((aPoint.Distance(aP1) < aTolerance) ||
       (aPoint.Distance(aP2) < aTolerance)) {
      XIntTools_Range aRange(anIt1.Value().Param(), theParamOnE);
	...
	XIntTools_ShrunkRange aSR (theEdge, aV1, aV2, aRange, theContext);
	...
	XBOPTools_Tools::UpdateVertex (theBC.Curve(), theParamOnCurve, aVertex);
	...
	if(nbbefore < nbafter) {
	  ...
	  XBOPTools_CArray1OfVSInterference& aVSs = theIntrPool->VSInterferences();
	  ...
	  if(!interffound) {
	    XBOPTools_CArray1OfVEInterference& aVEs = theIntrPool->VEInterferences();
	  ...
	  if(!interffound) {
	    XBOPTools_CArray1OfVVInterference& aVVs = theIntrPool->VVInterferences();
	  ...
	  if(!interffound) {
	    ...
	    if (!aFlag) {
	      //
	      // Add Interference to the Pool
	      XBOPTools_VSInterference anInterf (anIt1.Value().Index(), nF, aU, aV);
	      anIndexIn=aVSs.Append(anInterf);
	      //
	      // SetState for Vertex in DS;
	      theDS->SetState (anIt1.Value().Index(), XBooleanOperations_ON);
	      // Insert Vertex in Interference Object
	      XBOPTools_VSInterference& aVS = aVSs(anIndexIn);
	      aVS.SetNewShape(anIt1.Value().Index());

	      interffound = Standard_False;
	      const XBOPTools_ListOfInterference& aList1 =
		theIntrPool->InterferenceTable().Value(anIt1.Value().Index()).GetOnType(XBooleanOperations_EdgeSurface);
	      XBOPTools_ListOfInterference& amodifList1 = *((XBOPTools_ListOfInterference*)&aList1); // not very good approach
	      XBOPTools_ListIteratorOfListOfInterference anInterfIt(amodifList1);
		...
		if(anInterfIt.Value().With() == nF) {
	      ...
	      const XBOPTools_ListOfInterference& aList2 =
		theIntrPool->InterferenceTable().Value(nF).GetOnType(XBooleanOperations_EdgeSurface);
	      XBOPTools_ListOfInterference& amodifList2 = *((XBOPTools_ListOfInterference*)&aList2); // not very good approach
		...
		if(anInterfIt.Value().With() == anIt1.Value().Index()) {
	      ...
	      if(!interffound)
		theIntrPool->AddInterference(anIt1.Value().Index(), nF, XBooleanOperations_VertexSurface, anIndexIn);
	    }
	  }
...
void AddInterfForAdjacentFace(const Standard_Integer            theEdgeIndex,
  ...
  XBOPTools_CArray1OfSSInterference& aFFs = theIntrPool->SSInterferences();

  for(ffit = 1; ffit <= aFFs.Extent(); ffit++) {
    XBOPTools_SSInterference& aFFi3 = aFFs(ffit);
  ...
  if(bAddInterference) {
    XBOPTools_SSInterference anInterfSS (index1, index2, 1.e-07, 1.e-07, aCvs, aPnts);
    Standard_Integer anIndexInSS = aFFs.Append(anInterfSS);
    theIntrPool->AddInterference (index1, index2, XBooleanOperations_SurfaceSurface, anIndexInSS); 
  }
}
...
//purpose  : 
//=======================================================================
Standard_Boolean RejectPaveBlock(const XIntTools_Curve& theC,
                                 const Standard_Real   theT1,
                                 const Standard_Real   theT2,
                                 const TopoDS_Vertex&  theV,
                                 Standard_Real&        theRT)
{
  ...
  bClosed=XIntTools_Tools::IsClosed(aC);
  //
  isp=Standard_False;
  if (bIsPeriodic) {
    aPeriod=aC->Period();
...
//purpose  : 
//=======================================================================
Standard_Integer RejectBuildingEdge(const XIntTools_Curve& theC,
                                    const TopoDS_Vertex&  theV1,
                                    const TopoDS_Vertex&  theV2,
                                    const Standard_Real   theT1,
                                    const Standard_Real   theT2,
		   ...
		   Standard_Real& aTolR3D)
{
  ...
  TopoDS_Face aF[2];
  //
  XBooleanOperations_PShapesDataStructure myDS=aPF.DS();
  const Handle(XIntTools_Context)& myContext=aPF.Context();
  //
  aTolTresh=0.0005;
  ...
  //
  XBOPTools_SSInterference& aFFi=*((XBOPTools_SSInterference*)&aFF);
  XBOPTools_SequenceOfCurves& aSCvs=aFFi.Curves();
  aNbCurves=aSCvs.Length();
  if (aNbCurves!=1){
    return;
  ...
  for (i=0; i<2; ++i) {
    ...
    XBOPTools_Tools3D::GetNormalToSurface(aS[i], aU, aV, aDN[i]);
  }
  //
...
// purpose:
//=======================================================================
void XBOPTools_PaveFiller::PutClosingPaveOnCurve(XBOPTools_Curve& aBC,
						XBOPTools_SSInterference& aFFi)
{
  ...
  XBOPTools_PaveSet& aCPS=aBC.Set();
  //
  const XBOPTools_ListOfPave& aLP=aCPS.Set();
  aItLP.Initialize(aLP);
  for (; aItLP.More() && !bAdded; aItLP.Next()) {
    const XBOPTools_Pave& aPC=aItLP.Value();
    nVC=aPC.Index();
