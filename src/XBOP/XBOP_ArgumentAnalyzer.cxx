...
#include <XBOP_ArgumentAnalyzer.ixx>
#include <XBOP_CheckResult.hxx>
#include <XBOP_ShellSolid.hxx>
#include <XBOP_WireSolid.hxx>
#include <XBOP_WireShell.hxx>
...
static Standard_Boolean TestSubShapeType(const TopAbs_ShapeEnum theT1,
                                         ...
                                         const XBOP_Operation    theOP);

// ================================================================================
// function: Constructor
// purpose:
// ================================================================================
XBOP_ArgumentAnalyzer::XBOP_ArgumentAnalyzer() : 
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::SetShape1(const TopoDS_Shape & TheShape)
{
  myShape1 = TheShape;
}
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::SetShape2(const TopoDS_Shape & TheShape)
{
  myShape2 = TheShape;
}
...
// purpose:
// ================================================================================
const TopoDS_Shape & XBOP_ArgumentAnalyzer::GetShape1() const
{
  return myShape1;
}
...
// purpose:
// ================================================================================
const TopoDS_Shape & XBOP_ArgumentAnalyzer::GetShape2() const
{
  return myShape2;
}
...
// purpose:
// ================================================================================
XBOP_Operation& XBOP_ArgumentAnalyzer::OperationType() 
{
  return myOperation;
}
...
// purpose:
// ================================================================================
Standard_Boolean & XBOP_ArgumentAnalyzer::StopOnFirstFaulty()
{
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::Perform()
{
  try {
    OCC_CATCH_SIGNALS
    myResult.Clear();
...
//     }
    ...
    XBOP_CheckResult aResult;
    aResult.SetCheckStatus(XBOP_CheckUnknown);
...
// purpose:
// ================================================================================
Standard_Boolean XBOP_ArgumentAnalyzer::HasFaulty() const
{
...
// purpose:
// ================================================================================
const XBOP_ListOfCheckResult& XBOP_ArgumentAnalyzer::GetCheckResult() const
{
  return myResult;
}
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestTypes()
{
  Standard_Boolean isS1 = myShape1.IsNull(), isS2 = myShape2.IsNull();

  if(isS1 && isS2) {
    XBOP_CheckResult aResult;
    aResult.SetCheckStatus(XBOP_BadType);
    myResult.Append(aResult);
    return;
  }
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestSelfInterferences()
{
  ...
  for(i = 0; i < 2; i++) {
    ...
    if(aS.IsNull())
      continue;
    XBOPTools_Checker aChecker(aS);
    aChecker.Perform();

    if (aChecker.HasFaulty()) {
      const XBOPTools_ListOfCheckResults& aResultList = aChecker.GetCheckResult();
      XBOPTools_ListIteratorOfListOfCheckResults anIt(aResultList);

      for(; anIt.More(); anIt.Next()) {
	const XBOPTools_CheckResult& aCheckResult = anIt.Value();

	if((aCheckResult.GetCheckStatus() == XBOPTools_CHKUNKNOWN) ||
	   (aCheckResult.GetCheckStatus() == XBOPTools_BADSHRANKRANGE) ||
	   (aCheckResult.GetCheckStatus() == XBOPTools_NULLSRANKRANGE)) {
	  continue;
	}
	XBOP_CheckResult aResult;
	if(i == 0)
	...
	aResult.SetCheckStatus(XBOP_SelfIntersect);
	myResult.Append(aResult);
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestSmallEdge() 
{
  Standard_Integer i = 0;
...
//	    BRepExtrema_DistShapeShape aDist;
	...
	if(bKeepResult) {
	  ...
	  aResult.SetCheckStatus(XBOP_TooSmallEdge);
	  myResult.Append(aResult);
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestRebuildFace() 
{
  if((myOperation == XBOP_SECTION) ||
     (myOperation == XBOP_UNKNOWN))
    return;
  ...
  for(i = 0; i < 2; i++) {
	...
	aResult.SetCheckStatus(XBOP_NonRecoverableFace);
	myResult.Append(aResult);
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestTangent() 
{
 // not implemented
}
...
// purpose:
// ================================================================================
 void XBOP_ArgumentAnalyzer::TestMergeSubShapes(const TopAbs_ShapeEnum theType) 
{
  if(myShape1.IsNull() || myShape2.IsNull())
    return;

  XBOP_CheckStatus aStatus = XBOP_CheckUnknown;

  switch(theType) {
  case TopAbs_VERTEX: {
    aStatus = XBOP_IncompatibilityOfVertex;
    break;
  }
  case TopAbs_EDGE: {
    aStatus = XBOP_IncompatibilityOfEdge;
    break;
  }
  case TopAbs_FACE: {
    aStatus = XBOP_IncompatibilityOfFace;
    break;
  }
  default: 
    return;
  ...
  for(i = 1; i <= aSeq1.Length(); i++) {
	...
	if (aEE.IsDone()) {
	  ...
	  for (ii = 1; ii <= aCPrts.Length(); ii++) {
	    const XIntTools_CommonPrt& aCPart = aCPrts(ii);
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestMergeVertex() 
{
...
// purpose:
// ================================================================================
void XBOP_ArgumentAnalyzer::TestMergeEdge() 
{
...
// purpose:
// ================================================================================
// void XBOP_ArgumentAnalyzer::TestMergeFace() 
// {
  // not implemented
// }
...
Standard_Boolean TestShapeType(const TopoDS_Shape & TheShape)
{
  ...
  if(aT == TopAbs_COMPOUND && XBOPTools_Tools3D::IsEmptyShape(TheShape))
    return Standard_False;
  ...
  if(aT==TopAbs_COMPOUND || aT==TopAbs_COMPSOLID) {
    ...
    if(aT == TopAbs_COMPOUND) {
      if (aNbShapes==1) {
	TreatRes = XBOPTools_DSFiller::TreatCompound(TheShape, aSTmp);
	if(TreatRes != 0)
	  return Standard_False;
	aShape=aSTmp;
	aT = aShape.ShapeType();
...
{
  Handle(XIntTools_Context) aContext;
  TopoDS_Vertex aV1, aV2;
  //
  //modified by NIZNHY-PKV Tue Jan 31 15:07:13 2012f
  aContext=new XIntTools_Context;
  ...
  XIntTools_Range aRange(aFirst, aLast);
  XIntTools_ShrunkRange aSR(theEdge, aV1, aV2, aRange, aContext);
                                  ...
                                  const XBOP_Operation    theOP)
{
  ...
  if(theOP!=XBOP_UNKNOWN) {
    ...
    if((aT1==TopAbs_SHELL && aT2==TopAbs_SOLID) ||
      ...
      if(!XBOP_ShellSolid::CheckArgTypes(aT1,aT2,theOP)) {
        return Standard_False;
      }
    }
    if((aT1==TopAbs_WIRE && aT2==TopAbs_SOLID) ||
       (aT2==TopAbs_WIRE && aT1==TopAbs_SOLID)) {
      if(!XBOP_WireSolid::CheckArgTypes(aT1,aT2,theOP)) {
        return Standard_False;
      }
    }
    if((aT1==TopAbs_WIRE && aT2==TopAbs_SHELL) ||
       (aT2==TopAbs_WIRE && aT1==TopAbs_SHELL)) {
      if(!XBOP_WireShell::CheckArgTypes(aT1,aT2,theOP)) {
        return Standard_False;
      }
    }
  }
