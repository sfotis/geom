...
Standard_Integer GetIndex(const TopoDS_Shape& theShape, 
			  const XBooleanOperations_ShapesDataStructure& theDS);
...
static
  ...
  (const Standard_Integer theFaceIndex,
   ...
   const XBOPTColStd_IndexedDataMapOfIntegerIndexedMapOfInteger& theFFMap,
   const XBOPTools_DSFiller& theDSFiller,
...
Standard_Boolean IsEdgeValidForFace(const Standard_Integer theEdgeIndex,
				    const Standard_Integer theFaceIndex,
				    XBOPTools_SSInterference& theFF,
				    const XBOPTools_DSFiller& theDSFiller);
...
static
  TopAbs_State ComputeState(const TopoDS_Face& theF,
			  ...
			  const Handle(XIntTools_Context)& theCtx);
...
// purpose: 
//=================================================================================
void XBOP_SolidSolid::PrepareFaceSplits() 
{
  ...
  for (i=1; i<=aNb; i++) {
	...
	//
	pDS->SetState(aNewFaceIndex, XBooleanOperations_ON);
	...
	if(PropagateFaceStateByEdges(aShapeF, aMapOfEdgeIndex, aState)) {
	  ...
	  if(aState != TopAbs_ON) { // can not determine correctly ON state
	    XBooleanOperations_StateOfShape aConvertedState = XBOPTools_StateFiller::ConvertState(aState);
...
//  purpose:
// =====================================================================================================================
Standard_Boolean XBOP_SolidSolid::PropagateFaceStateByEdges(const TopoDS_Shape& theFace,
							   ...
							   TopAbs_State& theState) 
{
  ...
  XBooleanOperations_StateOfShape aFoundState = XBooleanOperations_ON;
  ...
  for(; anExpE.More(); anExpE.Next()) {
    ...
    XBooleanOperations_StateOfShape anEdgeState = aDS.GetState(nE);

    if((anEdgeState == XBooleanOperations_IN) ||
       (anEdgeState == XBooleanOperations_OUT)) {
  ...
  if(!bIsINOUT && bFoundNotON) {
    if(aFoundState == XBooleanOperations_IN)
      aState = TopAbs_IN;
    else if(aFoundState == XBooleanOperations_OUT)
      aState = TopAbs_OUT;     
  }
...
//purpose  : 
//=======================================================================
Standard_Boolean XBOP_SolidSolid::
...

{
  ...
  for (i=1; i<=aNb; i++) {
	...
	if(XBOPTools_Tools3D::CheckSameDomainFaceInside(aFace, aF2, pPaveFiller->Context())) {
	  theState = TopAbs_ON;
	  return Standard_True;
	}
      }
  ...
  if(!XBOPTools_Tools3D::ComputeFaceState(aFace, aRefSolid, pPaveFiller->Context(), aState)) {
    return Standard_False;
  }
  theState = aState;
...
//purpose  : 
//=======================================================================
Standard_Boolean XBOP_SolidSolid::TakeOnSplit(const Standard_Integer theFaceIndex,
					     const Standard_Integer theBaseFaceIndex) const
{
  ...
  if (!binout && !binin && !boutout) {
    ...
    //
    if (aState==TopAbs_IN) {
      if (myOperation==XBOP_FUSE || myOperation==XBOP_COMMON) {
			      ...
			      Standard_Boolean&            bFoundOUTOUT)
{
  ...
  XBooleanOperations_StateOfShape aStFa;
  //
  const XBooleanOperations_ShapesDataStructure& aDS = theDSFiller.DS();
  //
  ...
  for(; anItF.More(); anItF.Next()) {
	...
	//
	if(aStFa==XBooleanOperations_ON) {
	  aLisOfON.Append(nFa);
	}
	if(aStFa==XBooleanOperations_IN) {
	  aListOfIN.Append(nFa);
	}
	else if(aStFa==XBooleanOperations_OUT) {
  ...
  if (theFFMap.Contains(theBaseFaceIndex)) {
	...
	for(; anExpE.More(); anExpE.Next()) {
	  ...
	  if(bTestEdge) {
	    ...
	    bAnalytic = XBOPTools_Tools3D::TreatedAsAnalytic(aF2, aSp, aFace, 
							    aTolTangent, aTolR, 
	    ...
	    if(bAnalytic) {
	      ...
	      XBOPTools_Tools3D::GetBiNormal (aSp, aFace, aDBF1);
				    ...
				    const XBOPTools_DSFiller& theDSFiller)
{
  ...
  XBOPTools_SequenceOfCurves& aSCvs=theFF.Curves();
  Standard_Integer aNbCurves=aSCvs.Length();
  Standard_Integer i = 0;

  for (i=1; i<=aNbCurves; i++) {
    XBOPTools_Curve& aBC=aSCvs(i);
  ...
  for(i = 1; i <= aCBPool.Length(); i++) {
    const XBOPTools_ListOfCommonBlock& aCBList = aCBPool.Value(i);
    XBOPTools_ListIteratorOfListOfCommonBlock anItCB(aCBList);
	...
	const XBOPTools_PaveBlock& aPB2 = aCB.PaveBlock2();
			      ...
			      gp_Pnt& theP)
{
  ...
  //
  aP2D.Coord(aU, aV);
  XBOPTools_Tools2D::FaceNormal(aF, aU, aV, aDN);
  //
