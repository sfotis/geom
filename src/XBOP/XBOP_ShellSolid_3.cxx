...
//  purpose:
// =====================================================================================================================
Standard_Boolean XBOP_ShellSolid::SplitFace(const Standard_Integer theFaceIndex,
					   ...
					   TopTools_ListOfShape& theListOfFace) const
{
  ...
  for(; anExpE.More(); anExpE.Next()) {
    ...
    const XBOPTools_ListOfPaveBlock& aLPB = aSplitShapesPool(aDS.RefEdge(anEdgeIndex));
	...
	XBooleanOperations_StateOfShape aState = aDS.GetState(nSp);
	Standard_Boolean bAddToWes = ((aState == XBooleanOperations_IN) ||
				      (aState == XBooleanOperations_OUT));

	if(aState == XBooleanOperations_ON) {
	  bAddToWes = Standard_True;

	  XBOPTools_ListOfCommonBlock& aLCB = aCBPool(aDS.RefEdge(anEdgeIndex));
	  ...
	  for (; anItCB.More(); anItCB.Next()) {
	    ...
	    if(iCBFace == 0) {
	      XBOPTools_PaveBlock& aPBEF1 = aCB.PaveBlock1(anEdgeIndex);
	      XBOPTools_PaveBlock& aPBEF2 = aCB.PaveBlock2(anEdgeIndex);
	...
	if(bAddToWes) {
	  ...
	  if(nSpToAdd != nSp) {
	    ...
	    Standard_Boolean bToReverse=
	      XBOPTools_Tools3D::IsSplitToReverse1 (aSpF1, aSpF2, pPaveFiller->Context());
		...
		!BRep_Tool::IsClosed(aSpF2, aFace)) {
	      ...
	      if(!XBOPTools_Tools3D::DoSplitSEAMOnFace(aSpF2, aSpF1, aFace, bIsReversed))
		XBOPTools_Tools3D::DoSplitSEAMOnFace (aSpF2, aFace);
	    }
  ...
  for (i = 1; i <= aNb; i++) {
	...
	XBOPTools_ListOfCommonBlock& aLCB = aCBPool(aDS.RefEdge(anEdgeIndex2));
	...
	for (; anItCB.More(); anItCB.Next()) {
	  ...
	  if(iCBFace == theFaceIndex) {
	    XBOPTools_PaveBlock& aPB = aCB.PaveBlock1(anEdgeIndex2);
  ...
  XBOP_WireEdgeSet aFilteredWES (aFace);
