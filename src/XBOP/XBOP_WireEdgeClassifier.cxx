...
#include <XBOP_WireEdgeClassifier.ixx>
...
//=======================================================================
//function : XBOP_WireEdgeClassifier::XBOP_WireEdgeClassifier
//purpose  : 
//=======================================================================
  XBOP_WireEdgeClassifier::XBOP_WireEdgeClassifier (const TopoDS_Face& F,
						  const XBOP_BlockBuilder& BB)
:
  XBOP_CompositeClassifier(BB)
{
  myBCEdge.Face() = F;
}
...
//purpose  : 
//=======================================================================
  TopAbs_State XBOP_WireEdgeClassifier::Compare (const Handle(XBOP_Loop)& L1,
						const Handle(XBOP_Loop)& L2)
{ 
  ...
  if      ( isshape2 && isshape1 )  { 
    ...
    XBOP_BlockIterator Bit1 = L1->BlockIterator();
    Bit1.Initialize();
    yena1 = Bit1.More();
    while (yena1) {
      const TopoDS_Shape& s1 = 
	((XBOP_BlockBuilder*)myBlockBuilder)->Element(Bit1);
      const TopoDS_Shape& s2 = L2->Shape();
    ...
    const TopoDS_Shape& s1 = L1->Shape();
    ResetShape(s1);
    XBOP_BlockIterator Bit2 = L2->BlockIterator();
    for (Bit2.Initialize(); Bit2.More(); Bit2.Next()) {
      const TopoDS_Shape& s2 = 
	((XBOP_BlockBuilder*)myBlockBuilder)->Element(Bit2);
      CompareElement(s2);
    }
    state = State();
  }
...
//purpose  : 
//=======================================================================
  TopoDS_Shape XBOP_WireEdgeClassifier::LoopToShape(const Handle(XBOP_Loop)& L)
{
  myShape.Nullify();

  XBOP_BlockIterator Bit = L->BlockIterator();
  Bit.Initialize();
  if ( !Bit.More() ) {
    return myShape;
  }
  ...
  for  (; Bit.More(); Bit.Next()) {
    const TopoDS_Edge& E = 
      TopoDS::Edge(((XBOP_BlockBuilder*)myBlockBuilder)->Element(Bit));
    
    tolE = BRep_Tool::Tolerance(E);
    
    haspc = XBOPTools_Tools2D::HasCurveOnSurface (E, F);
    if (!haspc) {
      Handle(Geom2d_Curve) C2D;
      XBOPTools_Tools2D::CurveOnSurface (E, F, C2D, f, l, tolpc, Standard_False);
...
//purpose  : 
//=======================================================================
  TopAbs_State  XBOP_WireEdgeClassifier::CompareShapes (const TopoDS_Shape& B1, 
						       const TopoDS_Shape& B2)
{
  ...
  XIntTools_Tools::MakeFaceFromWireAndFace (aW1, theFace, aFN1);  
  XIntTools_Tools::MakeFaceFromWireAndFace (aW2, theFace, aFN2);  
  ...
  for (; anExp1.More(); anExp1.Next()){
    ...
    //                             PKV Thu Jun 13 09:09:53 2002
    {
      ...
      XBOPTools_Tools3D::OrientEdgeOnFace (aE1, aFF, aERight);
      
      dt2D=MinStepIn2d(aERight, aFF);
      XBOPTools_Tools3D::PointNearEdge (aERight, aFF, aT, dt2D, aP2DNear, aPNear);
    }
    
    //
//  Modified by skv - Thu Jul  6 12:01:17 2006 OCC12627 Begin
//     aSt=XIntTools_Tools::ClassifyPointByFace (aFN2, aP2DNear);
    Standard_Real     aTol = Precision::PConfusion();
    XIntTools_FClass2d aClass2d(aFN2, aTol);
...
//purpose  : 
//=======================================================================
  TopAbs_State  XBOP_WireEdgeClassifier::CompareElementToShape (const TopoDS_Shape& EE,
...
//purpose  : 
//=======================================================================
  void  XBOP_WireEdgeClassifier::ResetShape(const TopoDS_Shape& B)
{
...
//purpose  : 
//=======================================================================
  void  XBOP_WireEdgeClassifier::ResetElement(const TopoDS_Shape& EE)
{
  ...
  haspc = XBOPTools_Tools2D::HasCurveOnSurface(E, F); 
  if (!haspc) { 
    XBOPTools_Tools2D::CurveOnSurface(E, F, C2D, f2, l2, tolpc, Standard_True);
    tolE = BRep_Tool::Tolerance(E); 
    tol = Max(tolE,tolpc); 
    BB.UpdateEdge(E,C2D,F,tol); 
  } 
  ...
  XBOPTools_Tools2D::CurveOnSurface(E, F, C2D, f2, l2, tolpc, Standard_False);
  if (C2D.IsNull()) {
    Standard_ProgramError::Raise("WEC : ResetElement");
  }

  par = XBOPTools_Tools2D::IntermediatePoint (f2, l2);
...
//purpose  : 
//=======================================================================
  void XBOP_WireEdgeClassifier::CompareElement(const TopoDS_Shape& EE)
{
  const TopoDS_Edge& E = TopoDS::Edge(EE);
  const TopoDS_Face& F = myBCEdge.Face();
...
//purpose  : 
//=======================================================================
  TopAbs_State XBOP_WireEdgeClassifier::State()
{
