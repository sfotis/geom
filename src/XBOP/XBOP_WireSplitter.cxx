...
static
Standard_Boolean RecomputeAngles(const XBOP_ListOfEdgeInfo& aLEInfo, 
...
// purpose: 
//=======================================================================
  void XBOP_WireSplitter::DoWithFace()
{
  myEdges.Clear();
...
// purpose: 
//=======================================================================
  void XBOP_WireSplitter::DoWithListOfEdges(const TopTools_ListOfShape& aLE)
{
  myEdges.Clear();
			       ...
			       
{
  ...
  const XBOP_ListOfEdgeInfo& aLEInfoVb=mySmartMap.FindFromKey(aVb);
                   ...
                   const TopoDS_Face& aF)
{
  ...
  XBOPTools_Tools2D::CurveOnSurface 
                       ...
                       const Standard_Boolean aFlag)
{
  ...
  Handle(Geom2d_Curve) aC2D;
  Standard_Real aFirst, aLast, aToler;
  XBOPTools_Tools2D::CurveOnSurface (anEdge, myFace, aC2D, aFirst, aLast, aToler, Standard_True);
  if (aC2D.IsNull())
    return 0.;
...
Standard_Integer NbWaysOut(const XBOP_ListOfEdgeInfo& aLEInfo)
{
  ...
  XBOP_ListIteratorOfListOfEdgeInfo anIt(aLEInfo);
  for (; anIt.More(); anIt.Next()) {
    const XBOP_EdgeInfo& anEI=anIt.Value();
    //
    //const TopoDS_Edge& aE=anEI.Edge();
				 ...
				 TColStd_SequenceOfReal&   theRecomputedAngles)
{
  ...
  for (; anIt.More(); anIt.Next()) {
	...
	bRecomputeAngleLocal = Standard_True;
      ...
      if(!bRecomputeAngle) {
	XBOP_ListIteratorOfListOfEdgeInfo anIt2(aLEInfo);
	
	for(; anIt2.More(); anIt2.Next()) {
	  XBOP_EdgeInfo& anEI2=anIt2.Value();
