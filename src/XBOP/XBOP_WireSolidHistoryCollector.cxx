...
#include <XBOP_WireSolidHistoryCollector.ixx>
...
static Standard_Integer GetShapeWire(const XBooleanOperations_ShapesDataStructure& theDS,
...
// purpose:  
// ====================================================================================================
XBOP_WireSolidHistoryCollector::XBOP_WireSolidHistoryCollector(const TopoDS_Shape& theShape1,
...
// purpose:  
// ====================================================================================================
void XBOP_WireSolidHistoryCollector::SetResult(const TopoDS_Shape&       theResult,
...
// purpose:  
// ====================================================================================================
void XBOP_WireSolidHistoryCollector::FillSection(const XBOPTools_PDSFiller& theDSFiller) 
{
  // for edges
  ...
  for(i = 1; i <= nb; i++) {
    if(aDS.GetShapeType(i) != TopAbs_EDGE)
      continue;
    const XBOPTools_PaveSet& aPaveSet = aPavePool.Value(aDS.RefEdge(i));
    ...
    for(; anIt.More(); anIt.Next()) {
      ...
      if((aPave.Type() == XBooleanOperations_EdgeSurface) ||
	 (aPave.Type() == XBooleanOperations_EdgeEdge) ||
	 (aPave.Type() == XBooleanOperations_VertexEdge)) {
	XBOPTools_PShapeShapeInterference anInter = pIntrPool->GetInterference(aPave.Interference(), aPave.Type());
	...
	if(anInter != NULL) {
	  ...
	  if(aResultMap.Contains(aNewShape)) {
		...
		continue;
	      ...
	      if(aPave.Type() != XBooleanOperations_EdgeEdge) {
    ...
    for(j = 1; j <= aDS.NumberOfSuccessors(i); j++) {
      ...
      for (k = 1; k <= aNb; k++) {
	XBOPTools_VVInterference& VV=VVs(k);
	...
	for (k = 1; k <= aNb; k++) {
	  ...
	  if(aninterit == 0)
	    anInterference = (XBOPTools_ShapeShapeInterference*)(&pIntrPool->VEInterferences().Value(k));
	  else
	    anInterference = (XBOPTools_ShapeShapeInterference*)(&pIntrPool->VSInterferences().Value(k));
...
// ====================================================================================================
void XBOP_WireSolidHistoryCollector::FillEdgeHistory(const XBOPTools_PDSFiller& theDSFiller) 
{
  const XBooleanOperations_ShapesDataStructure& aDS = theDSFiller->DS();
  ...
  for(; iRank <= 2; iRank++) {
    XBooleanOperations_StateOfShape aStateCmp, aState;
    aStateCmp = XBOP_BuilderTools::StateToCompare(iRank, myOp);
    Standard_Integer i = 0;
	...
	for(; aPBIt.More(); aPBIt.Next()) {
	  ...
	  if ((aState == aStateCmp) || (myOp==XBOP_COMMON && (aState == XBooleanOperations_ON))) {
	    const TopoDS_Shape& aNewShape = aDS.Shape(nSp);

	    if(aResultMap.Contains(aNewShape)) {
	      Standard_Integer bIsGenerated = Standard_True;

	      if((myOp == XBOP_CUT) || (myOp == XBOP_FUSE)) {
      ...
      const XBOPTools_CommonBlockPool& aCBPool = aPaveFiller.CommonBlockPool();
      const XBOPTools_ListOfCommonBlock& aLCB = aCBPool(aDS.RefEdge(i));
...
Standard_Integer GetShapeWire(const XBooleanOperations_ShapesDataStructure& theDS,
			      const TopoDS_Shape& S1,
			      const TopoDS_Shape& S2,
			      TopoDS_Shape& WS)
{
